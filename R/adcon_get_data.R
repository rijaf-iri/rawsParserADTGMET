get.adcon.data <- function(aws_dir, adt_dir){
    ...
    for(j in seq_along(awsCrd$id)){
        ...
        for(mo in 1:(length(daty) - 1)){
            mm <- if(mo == 1) 0 else 1
            start_mo <- as.numeric(daty[mo] + mm)
            end_mo <- as.numeric(daty[mo + 1])
            ...
            out <- try(parse.adcon_db.data(qres, awsVAR1, awsID, awsNET), silent = TRUE)
...
parse.adcon_db.data <- function(tmp, awsVAR, awsID, awsNET){
    tz <- "Africa/Accra"
    origin <- "1970-01-01"
    ...
    tmp <- lapply(seq_along(awsVAR$node_var), function(i){
        ...
        x$measuringvalue <- as.numeric(x$measuringvalue) * v$multiplier
